...
def date_slider(first_video_date,last_video_date,date_end=datetime.today()):
...
if choice =="Home":
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file.name:
            ...
            Classification = st.button("Classification Musics!",key ='1')
            if Classification:
                ...
                selection_genre = st.selectbox('Select the genres', predict_list)
                
                # youtube =st.expander("Most song on Youtube!")
                if selection_genre:#youtube:
